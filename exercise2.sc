...
def findrecipe(y: List[String], r: List[Recipe]):List[Recipe] = {
  val recipes = r
  ...
  recipes.foreach { recipe => if(recipe.getIngredients.equals(y)) list = recipe::list }
  // return results
  list
}

// Testing -- return a list of recipes that match our ingredients
